...
class EnhancedDataExporter {
     ...
     */
    extractDetailedProps(component) {
     ...
     */
    extractEnhancedExamples(component) {
        const examples = [];
...
export default MyComponent;`,
    ...
    hasSimilarNaming(name1, name2) {
        const extractBase = (name) => name.replace(/(Button|Input|Field|Form|Modal|Table|Card)$/, '');
        return extractBase(name1) === extractBase(name2);
    }
    /**
     * Analyze component for usage patterns
     */
    extractUsagePatterns(component) {
        const patterns = [];
    ...
    getStyleRequirements(component) {
        const requirements = [];
        // Core CSS files - always required
        requirements.push('root.css', // CSS variables and global resets
        ...
        if (component.category === 'Forms') {
            requirements.push('forms.css', 'forms/forms-index.css', 'forms/input-fields.css', 'forms/submit-btn.css', 'forms/submit-button.css');
        }
        if (component.category === 'UI') {
            requirements.push('ui/ui-index.css', 'ui/card.css', 'ui/badge.css', 'ui/action.css', 'ui/tables.css', 'ui/charts.css', 'ui/dropdown-styles.css');
        }
        if (component.category === 'Navbar') {
            requirements.push('navbar/navbar-index.css', 'navbar/responsive-navbar.css', 'navbar/sidenav.css', 'navbar/breadcrumb.css', 'navbar/menu-dropdown.css');
        }
        if (component.category === 'Auth') {
            requirements.push('forms.css', 'forms/input-fields.css');
        }
        if (component.category === 'Status') {
            requirements.push('status/error-state.css', 'status/not-found.css', 'modals/modal.css');
        }
        if (component.category === 'Theme') {
            requirements.push('darkmode.css');
        }
        if (component.category === 'Tabs') {
            requirements.push('tabs/main.css', 'tabs/style.css');
        }
        ...
        if (component.tags?.includes('modal')) {
            requirements.push('modals.css', 'modals/modal.css', 'modals/style.css');
        }
        if (component.tags?.includes('table')) {
...
function DataTableExample() {
  ...
  return (
     ...
     */
    generateEnhancedSearchIndex(enhancedComponents) {
        const byName = {};
        const byCategory = {};
        const byTags = {};
        const byType = {};
    ...
    async export() {
        ...
        try {
            ...
            const enhancedComponents = this.components.map(component => this.enhanceComponentData(component));
            ...
            const enhancedExport = {
                metadata: {
                    ...
                    categories: Object.fromEntries(categories.map((cat) => [cat.name, {
                            name: cat.name,
                    ...
                    statistics: {
                        byType: Object.fromEntries(['component', 'hook', 'context', 'utility'].map(type => [
                            type,
...
//# sourceMappingURL=export-enhanced-data.js.map